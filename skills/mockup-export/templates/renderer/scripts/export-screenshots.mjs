...
await browser.close();
const slotCount = files.filter((f) => /^slot-\d+\.png$/.test(f)).length;
const hasFG = files.includes("feature-graphic.png");
console.log(`[export] done — ${slotCount} slot PNGs${hasFG ? " + feature-graphic.png (1024×500)" : ""} in ${OUT_DIR}`);
